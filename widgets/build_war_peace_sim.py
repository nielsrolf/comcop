...
who fights and who trades. Rendering conventions (as requested):

  * spikiness   -> weapon budget: a round blob = unarmed, a long-spiked star =
                   heavily armed (weapon_fraction near 1).
  * brightness  -> welfare: richer agents (more resources) glow brighter.
  * hue         -> heritable lineage colour, so kin clusters read as one shade.
...
OUT = os.path.join(ROOT, "assets", "war_peace_sim.html")
...
<style>
  ...
  button,select { border:1px solid #ccc; background:#f6f6f6; border-radius:6px; padding:6px 10px; cursor:pointer; font-size:13px; }
  button.active { background:#1f77b4; color:#fff; border-color:#1f77b4; }
  .wrap { display:flex; gap:14px; flex-wrap:wrap; }
  #stage { background:#0b0f19; border-radius:8px; }
  .side { min-width:230px; font-size:13px; }
  .stat { color:#333; margin:2px 0; }
...
<div class="wrap">
  ...
  <div class="side">
    <div id="stats"></div>
    <div id="inspect">Click an agent to inspect it.</div>
    <div class="legend">
      <b>How to read it</b><br>
      • spikier star = more <b>weapons</b> (round = disarmed)<br>
      • brighter = more <b>resources / welfare</b><br>
      • hue = heritable lineage colour<br>
      • <span style="color:#ff5252">red flash</span> = a fight this step<br>
      • <span style="color:#4caf50">green link</span> = a peaceful trade
    </div>
  </div>
</div>
...
function torus(a,g){let dx=Math.abs(a.x-g.x),dy=Math.abs(a.y-g.y);dx=Math.min(dx,1-dx);dy=Math.min(dy,1-dy);return dx*dx+dy*dy;}
...
function child(a){
  return {x:wrap(a.x+gauss(0,a.speed)),y:wrap(a.y+gauss(0,a.speed)),
          speed:Math.max(0,a.speed*gauss(1,0.1)),resources:EVOLVE,
          weapon:clip01(a.weapon+gauss(0,0.05)),aggr:clip01(a.aggr+gauss(0,0.05)),
...
function stepWorld(){
  ...
  for(const g of games){
    ...
    const [a,b]=pl;
    const atkA=Math.random()<a.aggr, atkB=Math.random()<b.aggr;
    if(atkA||atkB){
      let winner,loser;
  ...
  for(const a of agents){
    a.resources-=1;
    a.x=wrap(a.x+gauss(0,a.speed)); a.y=wrap(a.y+gauss(0,a.speed));
    if(a.resources<0) continue;
    while(a.resources>EVOLVE && next.length+agents.length<CAP_POP){next.push(child(a));a.resources-=EVOLVE;}
    next.push(a);
  }
  agents=next;
...
function drawStar(x,y,r,spikes,rot,fill){
  ...
  for(let i=0;i<spikes*2;i++){
    ...
    const ang=rot+Math.PI*i/spikes;
...
function render(){
  ...
  for(const l of links){
    ...
    ctx.strokeStyle=l.type==='fight'?'rgba(255,82,82,0.8)':'rgba(76,175,80,0.7)';
    ctx.lineWidth=l.type==='fight'?2.4:1.4; ctx.stroke();
  }
  for(const a of agents){
...
function stats(){
  ...
  $('stats').innerHTML=
    ...
    `<div class="stat"><b>Mean weapon budget:</b> ${mean('weapon').toFixed(2)}</div>`+
    `<div class="stat"><b>Mean aggression:</b> ${mean('aggr').toFixed(2)}</div>`;
  if(selected) inspect();
}
function inspect(){
  ...
  $('inspect').innerHTML=
    `<b>Agent ${a.id}</b><br>`+
    `resources (welfare): <b>${a.resources.toFixed(2)}</b><br>`+
    `weapon_fraction: <b>${a.weapon.toFixed(2)}</b><br>`+
    `aggression P(attack): <b>${a.aggr.toFixed(2)}</b><br>`+
    `speed: ${a.speed.toFixed(3)}<br>`+
    `neighbours within range: ${near}`;
}
cv.addEventListener('click',e=>{
  const rect=cv.getBoundingClientRect();
  const mx=(e.clientX-rect.left)/W, my=(e.clientY-rect.top)/H;
  let best=null,bd=1e9;
  for(const a of agents){const d=(a.x-mx)**2+(a.y-my)**2;if(d<bd){bd=d;best=a;}}
  if(bd<0.002){selected=best;} else selected=null;
  render(); inspect();
});
...
SRC = os.path.join(os.path.dirname(os.path.abspath(__file__)), "war_peace_sim.html")
